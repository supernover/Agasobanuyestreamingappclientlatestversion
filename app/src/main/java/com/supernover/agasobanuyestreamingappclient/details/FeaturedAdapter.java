...
public class FeaturedAdapter extends RecyclerView.Adapter<FeaturedAdapter.viewHolder> {
    private List<FeaturedModel> modelList;

    public FeaturedAdapter(List<FeaturedModel> modelList2) {
    ...
    public void onBindViewHolder(viewHolder holder, int position) {
        holder.setData(this.modelList.get(position).getFcast(), this.modelList.get(position).getTlink(), this.modelList.get(position).getFcover(), this.modelList.get(position).getFlink(), this.modelList.get(position).getFrating(), this.modelList.get(position).getFstudio(), this.modelList.get(position).getFthumb(), this.modelList.get(position).getFdesc(), this.modelList.get(position).getFtitle());
    }
    ...
    public class viewHolder extends RecyclerView.ViewHolder {
        ...
        private void setData(final String f_cast, final String t_link, final String f_cover, final String f_link, final String f_rating, final String f_studio, final String f_thumb, final String f_des, final String f_title) {
            this.title.setText(f_title);
            Glide.with(this.itemView.getContext()).load(f_thumb).into(this.imageView);
            this.itemView.setOnClickListener(new View.OnClickListener() {
                /* class com.birjulabsinc.birjutv.details.FeaturedAdapter.viewHolder.AnonymousClass1 */
                ...
                public void onClick(View v) {
                    ...
                    featuredDetails.putExtra("f_rating", f_rating);
                    featuredDetails.putExtra("f_studio", f_studio);
